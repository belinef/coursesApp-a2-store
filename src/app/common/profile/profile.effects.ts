...
export class ProfileEffects {
  ...
  avatarUpdating$ = this.actions$
    ...
    .switchMap((avatar) => {
      ...
      return this.profileService.updateProfile('avatar', result)
        .map(() => new ConfirmAvatarImage(result));
  });
  ...
  dateofBirthUpdating$ = this.actions$
    ...
    .switchMap(({ data: {formatted} }) => {
      ...
      return this.profileService.updateProfile('dateOfBirth', formatted)
        .map(() => new BirthDateConfirmed());
  });
}
